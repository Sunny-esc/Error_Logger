...
const Login = ({ onClose}) => {
  ...
  <div className={`${size} border-4 border-gray-200 border-t-4 ${color} rounded-full animate-spin`}></div>
);
 ...
 // or use cookies if your backend sets them
    ...
    try {
      ...
      toast.success("Login successful! ");
          ...
          Auth.isUser = true;
      setTimeout(() => navigate(from ||'/dashboard', { replace: true }), 2000);
      if (onClose) onClose(); // Close popup after login
    } catch (error) {
        ...
        <form onSubmit={onlogin}>
          ...
          <p className="text-sm md:text-lg mb-4">
            Don’t have an account?{" "}
             ...
             <button onClick={()=>navigate('/sign') }className="w-9 text-lg text-blue-400 underline hover:text-blue-600 ">sign</button> 
